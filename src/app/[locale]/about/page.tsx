...
function renderBio(paragraphs: string[]) {
  return paragraphs.map((para, i) => {
    ...
    return (
      <p
        key={i}
        className={`leading-relaxed ${i === 0 ? "text-lg text-gray-700 dark:text-gray-200" : "text-gray-600 dark:text-gray-300"}`}
      >
        {parts.map((part, j) =>
          j % 2 === 1 ? (
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-indigo-50/60 via-white to-white dark:from-indigo-950/30 dark:via-background dark:to-background">
          ...
          <div className="relative max-w-4xl mx-auto px-4 py-16 flex flex-col md:flex-row gap-10 md:gap-14 items-center">
            <div className="relative w-44 h-44 flex-shrink-0 rounded-full ring-4 ring-brand/20 shadow-xl overflow-hidden">
              <Image src={aboutData.image} alt={aboutData.name} fill className="object-cover" sizes="176px" priority />
            </div>
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {aboutData.headline.map((h) => (
                    <span key={h} className="px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200/60 dark:border-indigo-700/50">
                      {h}
                    </span>
                  ))}
                </div>
              ...
              <a
                ...
                className="inline-flex items-center gap-2 w-fit px-5 py-2.5 bg-[#0077B5] hover:bg-[#006097] text-white rounded-full text-sm font-semibold transition-colors"
              >
                <LinkedInIcon />
                {t("viewLinkedIn")}
              </a>
        ...
        {/* ── Bio ───────────────────────────────────────────── */}
        <section className="max-w-3xl mx-auto px-4 py-14 flex flex-col gap-5">
            ...
            {aboutData.education.map((edu, i) => (
              <div key={i} className="relative pl-8 border-l-2 border-brand/20">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  {edu.badge && (
                    <span className="inline-flex w-fit items-center gap-1 mt-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200/60 dark:border-amber-700/40">
                      ★ {edu.badge}
                    </span>
                  )}
                  {edu.incomplete && (
